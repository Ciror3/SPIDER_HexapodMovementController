...
def build_previous_commands(calibration_path=None):
    if calibration_path is not None:
        from spider_rl.calibration import build_previous_commands as load_calibration

        commands = load_calibration(calibration_path)
        expected_sources = {INITIAL_PREVIOUS_MOTION} | {
            motion_name for _, motion_name in ACTION_METADATA.values()
        }
        missing = []
        for source in sorted(expected_sources):
            missing_actions = sorted(set(ACTION_METADATA).difference(commands.get(source, {})))
            if missing_actions:
                missing.append(f"{source}: {missing_actions}")
        if missing:
            details = "; ".join(missing)
            raise ValueError(f"La calibración no cubre todas las transiciones: {details}")
        return commands
...
class SpiderEnv(gym.Env):
    ...
    def __init__(
        ...
        obs_low = [np.full(2, -np.inf, dtype=np.float32)]
        obs_high = [np.full(2, np.inf, dtype=np.float32)]
        ...
        if target_init_pos is None:
            self.target_pos = np.array(
                [
                    self.np_random.uniform(-half_x, half_x),
                    self.np_random.uniform(-half_y, half_y),
                ],
                dtype=np.float64,
            )
        else:
    ...
    def render(self):
        if self.render_mode != "human":
            return
        if pygame is None:
            raise ModuleNotFoundError("Instalá pygame para usar render_mode='human'.")
