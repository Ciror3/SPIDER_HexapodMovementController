...
class SpiderEnv(gym.Env):
    ...
    def __init__(
        ...
        target_low = np.full(2, -np.inf, dtype=np.float32)
        target_high = np.full(2, np.inf, dtype=np.float32)
    ...
    def _sample_obstacles(self, target_pos):
        ...
        for _ in range(self.num_obstacles):
            ...
            for _ in range(max_attempts):
                ...
                x = float(self.np_random.uniform(-half_x + r, half_x - r))
                y = float(self.np_random.uniform(-half_y + r, half_y - r))
                center = np.array([x, y])
