...
class SpiderEnv(gym.Env):
    ...
    def __init__(
        ...
        self.observation_space = gym.spaces.Box(
            low=np.concatenate(
                [
                    np.full(2, -np.inf, dtype=np.float32),  # target escalado
                    np.full(2 * self.obs_max_obstacles, -np.inf, dtype=np.float32),
        ...
        if target_init_pos is None:
            self.target_pos = np.array(
                [
                    self.np_random.uniform(-self.half_extents[0], self.half_extents[0]),
                    self.np_random.uniform(-self.half_extents[1], self.half_extents[1]),
                ],
                dtype=np.float64,
            )
        else:
    ...
    def _sample_obstacles(self, target_pos):
        ...
        for _ in range(n_to_place):
            ...
            for _ in range(max_attempts):
                ...
                alpha = float(self.np_random.uniform(0.2, 0.8))
